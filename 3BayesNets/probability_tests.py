...
class ProbabilityTests(unittest.TestCase):
    ...
    def test_probability_setup(self):
        ...
        # test H distribution͏󠄂͏️͏󠄌͏󠄎͏︀͏︄͏󠄉
        ...
        # test C distribution͏󠄂͏️͏󠄌͏󠄎͏︀͏︄͏󠄉
        security_system = set_probability(make_security_system_net())
        ...
        # test M distribution͏󠄂͏️͏󠄌͏󠄎͏︀͏︄͏󠄉
        security_system = set_probability(make_security_system_net())
        ...
        # test B distribution͏󠄂͏️͏󠄌͏󠄎͏︀͏︄͏󠄉
        security_system = set_probability(make_security_system_net())
        ...
        # Q distribution͏󠄂͏️͏󠄌͏󠄎͏︀͏︄͏󠄉
        # can't test exact probabilities because͏󠄂͏️͏󠄌͏󠄎͏︀͏︄͏󠄉
        # order of probabilities is not guaranteed͏󠄂͏️͏󠄌͏󠄎͏︀͏︄͏󠄉
    ...
    #Part 1c Test͏󠄂͏️͏󠄌͏󠄎͏︀͏︄͏󠄉
    def test_marginal(self):
        """Test the marginal probability of Double-0 getting compromised."""
        security_system = set_probability(make_security_system_net())
        marginal_prob = get_marginal_double0(security_system)
        print(marginal_prob)
        conditional_prob1 = get_conditional_double0_given_no_contra(security_system)
        print(conditional_prob1)
        conditional_prob2 = get_conditional_double0_given_no_contra_and_bond_guarding(security_system)
        print(conditional_prob2)

    #Part 2a Test͏󠄂͏️͏󠄌͏󠄎͏︀͏︄͏󠄉
    def test_games_network(self):
        ...
        # Now testing that all nodes in the games network have proper probability distributions.͏󠄂͏️͏󠄌͏󠄎͏︀͏︄͏󠄉
        # Note that all nodes have to be named predictably for tests to run correctly.͏󠄂͏️͏󠄌͏󠄎͏︀͏︄͏󠄉

        # First testing team distributions.͏󠄂͏️͏󠄌͏󠄎͏︀͏︄͏󠄉
        # You can check this for all teams i.e. A,B,C (by replacing the first line for 'B','C')͏󠄂͏️͏󠄌͏󠄎͏︀͏︄͏󠄉
        ...
        # Now testing match distributions.͏󠄂͏️͏󠄌͏󠄎͏︀͏︄͏󠄉
        # You can check this for all matches i.e. AvB,BvC,CvA (by replacing the first line)͏󠄂͏️͏󠄌͏󠄎͏︀͏︄͏󠄉
    ...
    #Part 2c Test͏󠄂͏️͏󠄌͏󠄎͏︀͏︄͏󠄉
    def test_gibbs_sampler(self):
        gibbs_samples = []
        gibbs = []
        for _ in range(20000):
            gibbs = Gibbs_sampler(get_game_network(), gibbs)
            gibbs_list = list(gibbs)
            gibbs_samples.append(gibbs_list)
        BvC = []
        count_0 = 0
        count_1 = 0
        count_2 = 0
        for gibbs in gibbs_samples:
            if gibbs[4] == 0:
                count_0 += 1
            elif gibbs[4] == 1:
                count_1 += 1
        ...
        posterior = [count_0/20000, count_1/20000, count_2/20000]
        print(posterior)
        self.assertTrue(abs(posterior[0]-0.25)<0.01 and abs(posterior[1]-0.42)<0.01 and abs(posterior[2]-0.31)<0.01, msg='Incorrect posterior calculated')

    #Part 2d Test͏󠄂͏️͏󠄌͏󠄎͏︀͏︄͏󠄉
    def test_mcmc_inference(self):
        MH_samplers = []
        gibbs = []
        for _ in range(10000):
            gibbs = MH_sampler(get_game_network(), gibbs)
            gibbs = list(gibbs)
            MH_samplers.append(gibbs)
        BvC = []
        count_0 = 0
        count_1 = 0
        count_2 = 0
        for gibbs in MH_samplers:
            if gibbs[4] == 0:
                count_0 += 1
            elif gibbs[4] == 1:
                count_1 += 1
        ...
        print(BvC)
        print(count_0, count_1, count_2)
        posterior = [count_0/10000, count_1/10000, count_2/10000]
        print(posterior)
        self.assertTrue(abs(posterior[0]-0.25)<0.01 and abs(posterior[1]-0.42)<0.01 and abs(posterior[2]-0.31)<0.01, msg='Incorrect posterior calculated')

    #Part 2e Test͏󠄂͏️͏󠄌͏󠄎͏︀͏︄͏󠄉
    def test_compare(self):
        # result_list = [0, 0, 0]
        # for _ in range(10):
        #     _,_,gibbs_count, mh_count, _ = compare_sampling(get_game_network(), [3,3,3,0,0,2])
        ...
        #         result_list[2] += 1
        # print(result_list)
        result = compare_sampling(get_game_network(), [0,3,3,0,0,2])
        print(result)
